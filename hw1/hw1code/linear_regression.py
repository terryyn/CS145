...
def getBeta(train_x, train_y):   
    ...
    left_operand = np.linalg.inv(np.matmul(x_transpose, train_x))
    right_operand = np.matmul(x_transpose, train_y)
    beta = np.matmul(left_operand, right_operand)
    ...
    for iter in range(0, num_iter):
       ...
       for i in range(n):
           ...
           #========================#
            ...
            # gradient of OLS(1/2 (yi-xi)^2) 
            cur_deriv = train_x[i] * (np.dot(beta, np.transpose(train_x[i])) - train_y[i])
            deriv = np.add(deriv, cur_deriv)
...
def getBetaStochasticGradient(train_x, train_y, lr):
    ...
    for iter in range(epoch):
        ...
        for i in range(n):
           ...
           # use np.multiply instead of * here to avoid overflow
           # needs to multiply lr first to avoid overflow
            coefficient = lr * (train_y[idx] - np.dot(np.transpose(train_x[idx]), beta))
            cur_update = np.multiply(coefficient, train_x[idx])
            beta = np.add(beta, cur_update)
...
class LinearRegression(object):
    ...
    def train(self, algType):
        ...
        if(algType == '0'):
            ...
            # change learning rate to 0.0005 to converge
            beta = getBetaStochasticGradient(newTrain_x, self.train_y.values, 0.0005)
            #print('Beta: ', beta)
        else:
