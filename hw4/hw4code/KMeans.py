...
def compute_purity(clusters,total_points):
    ...
    for i in range(num_clusters):
        cluster = clusters[i]
        original_labels = [x.label for x in cluster]
        max_union = max(original_labels, key=original_labels.count)
        maxLabelCluster.append(max_union)
    # ========================#
    #   END YOUR CODE HERE   #
    # ========================#
    purity = 0.0
...
def calcNMI(nmiMatrix):
    ...
    for i in range(row - 1):
        for j in range(col - 1):
            ...
            # stores total count in last row and column
            logPart_I = N * float(nmiMatrix[i][j]) / (float(nmiMatrix[i][-1]) * nmiMatrix[-1][j])
        ...
        # STRART YOUR CODE HERE  #
        # ========================#
        p_wj = nmiMatrix[i][-1] / float(N)
        HOmega += (p_wj * np.log(p_wj))
        # ========================#
        #   END YOUR CODE HERE   #
        # ========================#
...
class KMeans:
    ...
    def getCentroid(self, cluster):
        ...
        # STRART YOUR CODE HERE  #
        # ========================#
        cx = np.average([c.x for c in cluster])
        cy = np.average([c.y for c in cluster])
        # ========================#
        #   END YOUR CODE HERE   #
        # ========================#
