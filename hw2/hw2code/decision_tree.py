...
def compute_info_gain(data, att_name, y_name):
    ...
    #========================#
    info = compute_entropy(data[y_name])
    entropy = 0
    for i in range(len(vals)):
        probability = counts[i] / total_counts
        val_data = data.loc[data[att_name] == vals[i]]
        entropy += probability * compute_entropy(val_data[y_name])
    info_gain = info - entropy
    #========================#
    #   END YOUR CODE HERE   #
    #========================# 
...
def comput_gain_ratio(data, att_name, y_name):
    ...
    for i in range(len(counts)):
        probability = counts[i] / total_counts
        att_info -= probability * np.log2(probability)
    #========================#
    #   END YOUR CODE HERE   #
    #========================# 
...
class DecisionTree(object):
    ...
    def make_tree(self, train_data, parent_node_class = None):
        ...
        if len(np.unique(data[self.y_name])) <= 1:
            ...
            # STRART YOUR CODE HERE  #
            #========================#
            leaf_value = data[self.y_name].values[0]
            #========================#
            #   END YOUR CODE HERE   #
            #========================# 
            return leaf_value
