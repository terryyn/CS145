...
def polynomial_kernel(X, Y=None, degree=3):
    ...
    for i in range(m):
        for j in range(n):
            kernel_matrix[i,j] = (1+np.dot(X[i],Y[j])) ** degree
    #========================#
    #   END YOUR CODE HERE   #
    #========================# 
    return kernel_matrix
...
def gaussian_kernel(X, Y=None, sigma=5.0):
    ...
    for i in range(m):
        for j in range(n):
            kernel_matrix[i,j] = np.exp((-np.linalg.norm(X[i]-Y[j])**2)/ (2*(sigma**2)))
    #========================#
    #   END YOUR CODE HERE   #
    #========================# 
    return kernel_matrix
...
class SVM(object):
    ...
    def predict(self, X):
        if self.w is not None: ## linear case
            ...
            # STRART YOUR CODE HERE  #
            #========================#
            predicted_y = np.dot(self.w, np.transpose(X)) + self.b
            #========================#
            #   END YOUR CODE HERE   #
            #========================# 
            return predicted_y
            ...
            # STRART YOUR CODE HERE  #
            #========================#
            kernel_result = self.kernel(X,self.sv)
            for i in range(n):
                for j in range(self.sv.shape[0]):
                    predicted_y[i] += self.a[j]*self.sv_y[j]*kernel_result[i,j]
            #========================#
            #   END YOUR CODE HERE   #
            #========================# 
            return predicted_y
